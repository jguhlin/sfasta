...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("get_masking_ranges", |b| b.iter(|| get_masking_ranges(black_box(seq.as_bytes()))));
    group.bench_function("get_masking_ranges_previous", |b| b.iter(|| get_masking_ranges_previous(black_box(seq.as_bytes()))));
    group.finish();

    let ranges = get_masking_ranges(seq.as_bytes());

    let mut group = c.benchmark_group("Convert Ranges to ml32bit");
    group.throughput(criterion::Throughput::Bytes((std::mem::size_of::<usize>() * 2 * ranges.len()) as u64));
    ...
    group.throughput(criterion::Throughput::Bytes((std::mem::size_of::<Ml32bit>() * ml32bit.len()) as u64));
    ...
    group.throughput(criterion::Throughput::Bytes((std::mem::size_of::<u32>() * u32s.len()) as u64));
    ...
    group.throughput(criterion::Throughput::Bytes((seq.len() + std::mem::size_of::<Ml32bit>() * commands.len()) as u64));
