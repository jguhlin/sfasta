...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("pad_commands_to_u32", |b| b.iter(|| pad_commands_to_u32(black_box(&ml32bit))));

    let ml32bit_padded = pad_commands_to_u32(&ml32bit);

    c.bench_function("convert_commands_to_u32", |b| b.iter(|| convert_commands_to_u32(black_box(&ml32bit_padded))));

    let u32s = convert_commands_to_u32(&ml32bit_padded);

    c.bench_function("convert_u32_to_commands", |b| b.iter(|| convert_u32_to_commands(black_box(&u32s))));
