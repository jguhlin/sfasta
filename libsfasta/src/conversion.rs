...
impl Converter {
    ...
    pub fn with_threads(mut self, threads: usize) -> Self {
        assert!(threads < u16::MAX as usize, "Maximum number of supported threads is u16::MAX");
        ...
        R: BufRead + Read + Send,
    {
        ...
        let mut seqlocs_blocks_locs: Vec<u64> = Vec::with_capacity(seq_locs.len() / SEQLOCS_CHUNK_SIZE);
            ...
            .chunks(SEQLOCS_CHUNK_SIZE)
        {
            ...
            seqlocs_blocks_locs.push(out_fh
                .seek(SeekFrom::Current(0))
                .expect("Unable to work with seek API"));
        ...
        if self.index {
            ...
            let seqlocs_blocks_locs: Vec<u64> = seqlocs_blocks_locs.into_iter().map(|x| x - seqlocs_loc).collect();
